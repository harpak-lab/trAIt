...
def _norm(s):
    """Normalize integer-formatted floats: '1.0' → '1', '0.0' → '0'.
    Handles pandas dtype mismatch where the answer key reads Egg Style as
    object/int ('0','1') but the pred CSV (with NaN cells) reads as float64."""
    try:
        f = float(s)
        if f == int(f):
            return str(int(f))
